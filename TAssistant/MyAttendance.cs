...
namespace TAssistant
{
    public partial class MyAttendance : Form
    {
        ...
        {
            int adid = MyVar.uid;
            string classDate = DateTime.Now.ToString("yyyy-MM-dd");
            SqlConnection conn = new SqlConnection();
            conn.ConnectionString = @"Data Source=DESKTOP-LDSCKMJ;Initial Catalog=TAssistant;User ID=sa;Password=1234";
            SqlCommand command = new SqlCommand();
            command.Connection = conn;
            command.CommandText = "SELECT * FROM myattendance WHERE adminId = '" + adid + "'AND classdate='" + classDate + "'";
            DataTable data = new DataTable();
            SqlDataAdapter adapter = new SqlDataAdapter(command);
            adapter.Fill(data);
            dataGridViewMyAttendance.DataSource = data;
        }
        ...
        private void SaveData_Click(object sender, EventArgs e)
        {
            ...
            string classDate = DateTime.Now.ToString("yyyy-MM-dd");
            string present = comboBoxPresent.Text;
            string courseNo = textBoxCourseNo.Text;

            if (stuId == "" || dept == "" || stuYear == "" || stuSem == "" || courseNo == ""||present=="")
            {
                MessageBox.Show("You have to fill all the fields.", "Message", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            else
            {
                AdminDAO saveAttenData = new AdminDAO();
                int adminId = MyVar.uid;
                saveAttenData.AddAttendanceData(new MyInfoDTO(stuId, dept, stuYear, stuSem, classDate, present, courseNo, adminId));

                textBoxStudentId.Text = "";
                comboBoxPresent.Text = "";
            }
        ...
        private void UpdateDataBtn_Click_1(object sender, EventArgs e)
        {
            ...
            {
                AdminDAO updateAttendata = new AdminDAO();
                int adminId = MyVar.uid;//static value from my var class
                updateAttendata.UpdateAttendanceData(new MyInfoDTO(stuId, dept, stuYear, stuSem, classDate,present, courseNo, adminId));
